...
public class AiService {
    ...
    public ChatResponse chat(ChatRequest request, Long farmerId, String token) {
        ...
        if (geminiApiKey == null || geminiApiKey.trim().isEmpty()) {
            ...
            return new ChatResponse(
                    ...
                    + "[LIVE GEMINI TEST BLOCKED — GEMINI_API_KEY NOT CONFIGURED]"
            );
        }
        ...
        if (farmerId != null && token != null) {
            ...
            try {
                farmer = farmerClient.getFarmerProfile(token, farmerId);
            } catch (Exception e) {
            ...
            try {
                score = sustainabilityClient.getScore(token, farmerId);
            } catch (Exception e) {
            ...
            try {
                learning = learningClient.getLearningSummary(token, farmerId);
            } catch (Exception e) {
            ...
            try {
                modules = learningClient.getPublishedModules(token);
            } catch (Exception e) {
                log.error("Failed to fetch Published Modules: {}", e.getMessage());
            }

            contextPrompt =
                    "You are the FarmXP AI Assistant.\n" +
            ...
            if (score != null && score.getTotalScore() != null) {
                contextPrompt += "Sustainability Score: "
                        + score.getTotalScore() + "\n";
            }

            if (learning != null && learning.getCompletedModules() != null) {
                contextPrompt += "Completed Modules Count: "
                        + learning.getCompletedModules() + "\n";
            }
        ...
        try {
                    ...
                    "contents", List.of(
                            Map.of(
                                    "parts", List.of(
                                            Map.of(
                                                    "text",
                                                    contextPrompt + request.getMessage()
                                            )
                                    )
                            )
                    )
            ...
            log.info("Calling Gemini 3.5 Flash Lite...");
            ...
            return new ChatResponse(
                    "⚠️ AI service configuration error (Invalid API Key)."
            );

        } catch (HttpClientErrorException.TooManyRequests e) {

            return new ChatResponse(
                    "⚠️ The AI service is currently busy. Please try again later."
            );
            ...
            return new ChatResponse(
                    "⚠️ Gemini error: " + e.getStatusCode()
            );
    ...
    public RecommendationResponse recommendation(
        ...
        String prompt =
                "Give a brief farming recommendation for "
                + request.getCropName()
                + " during "
                + request.getSeason()
                + " season in "
    ...
    public MlRecommendationResponse getMlRecommendation(
            Long farmerId,
            String token) {
        ...
        try {
            ...
            log.error(
                    "Failed to fetch Farmer Profile for farmerId {}: {}",
                    farmerId,
                    e.getMessage()
            );
        }
        ...
        try {
            ...
            log.error(
                    "Failed to fetch Learning Summary for farmerId {}: {}",
                    farmerId,
                    e.getMessage()
            );
        }
        ...
        try {
            ...
            log.error(
                    "Failed to fetch Sustainability Score for farmerId {}: {}",
                    farmerId,
                    e.getMessage()
            );
        }
        ...
        try {
            ...
            log.error(
                    "Failed to fetch Published Modules: {}",
                    e.getMessage()
            );
        }
        ...
        Map<String, Object> farmerContext =
        ...
        farmerContext.put(
                "xp",
                farmer != null && farmer.getTotalXp() != null
                        ? farmer.getTotalXp()
                        : 0
        );
        ...
        farmerContext.put(
                "completedModules",
                learning != null
                        && learning.getCompletedModules() != null
                        ? learning.getCompletedModules()
                        : 0
        );
        ...
        try {
            ...
            MlRecommendationResponse fallback =
                    new MlRecommendationResponse();

            fallback.setFarmerId(farmerId);
            fallback.setCluster("BEGINNER");
            fallback.setRecommendedModuleId(1L);
            fallback.setRecommendedModuleTitle(
                    "Getting Started with Sustainability"
            );
            fallback.setReason(
                    "Our ML engine is currently initializing. "
                    + "Here's a great starter module!"
            );
            fallback.setImpactScore(10.0);

            fallback.setExpectedBenefits(
                    Map.of(
                            "water", "Medium",
                            "chemical", "Medium",
                            "yield", "Medium"
